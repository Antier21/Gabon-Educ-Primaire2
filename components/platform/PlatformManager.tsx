...
function SubjectsView({ workspace, classes, persist }: ViewProps) {
  const [section, setSection] = useState<"assignments" | "subjects">("assignments");
  /**
  ...
  /**
   * Au primaire, un titulaire produit techniquement une affectation par
   * matière. L'interface les présente pourtant comme une seule décision :
   * « cette personne tient cette classe ». Le retrait suit la même logique et
   * supprime le lot, au lieu d'obliger l'administration à cliquer quinze fois.
   */
  async function removePrimaryClassTeacher(classId: string) {
    const linked = workspace.assignments.filter(
      (item) => item.active && item.classId === classId && item.headTeacher,
    );
    if (!linked.length) return;
    const selectedClass = classes.find((item) => item.id === classId);
    if (!confirm(`Retirer le titulaire de la classe ${selectedClass?.name || "sélectionnée"} ?`)) return;
    await persist(
      {
        ...workspace,
  ...
  const eligibleTeachers = workspace.users.filter(
    (item) =>
      ["teacher", "head_teacher"].includes(item.role) &&
      item.status === "active" &&
      item.invitationStatus === "accepted",
  );
  const primary = workspace.school?.schoolType === "primary";
  const classesWithTitular = new Set(
    activeAssignments.filter((item) => item.headTeacher).map((item) => item.classId),
  );
  const exceptions = activeAssignments.filter((item) => !item.headTeacher);
  ...
  return (
    <>
      ...
      <div className={styles.subjectTabs} role="tablist" aria-label="Matières et affectations">
        <button
          type="button"
          role="tab"
          aria-selected={section === "assignments"}
          className={section === "assignments" ? styles.subjectTabActive : styles.subjectTab}
          onClick={() => setSection("assignments")}
        >
          Affectations
          <span>{primary ? classesWithTitular.size : activeAssignments.length}</span>
        </button>
        <button
          type="button"
          role="tab"
          aria-selected={section === "subjects"}
          className={section === "subjects" ? styles.subjectTabActive : styles.subjectTab}
      ...
      {section === "assignments" ? (
        <div className={styles.assignmentWorkspace}>
          <section className={styles.assignmentIntro}>
            <div>
              <small>Organisation pédagogique</small>
              <h2>Affecter les enseignants aux classes</h2>
              <p>
                {primary
                  ? "Choisissez d’abord le titulaire de chaque classe. Ajoutez ensuite uniquement les interventions spécialisées."
          ...
          {primary ? (
            <div className={styles.assignmentForms}>
              <form className={styles.assignmentCard} onSubmit={assignPrimaryClassTeacher}>
                <span className={styles.assignmentStep}>1</span>
                <div className={styles.assignmentCardHead}>
                  <h3>Définir le titulaire</h3>
                  <p>Un seul choix suffit pour toute la classe.</p>
                </div>
                <label>Classe<select name="classId" required><option value="">Sélectionner une classe</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
                <label>Enseignant titulaire<select name="teacherId" required><option value="">Sélectionner un enseignant</option>{eligibleTeachers.map((item) => <option key={item.id} value={item.id}>{item.firstName} {item.lastName}</option>)}</select></label>
                <button className={styles.assignmentPrimaryButton}>Enregistrer le titulaire</button>
              </form>

              <form className={styles.assignmentCard} onSubmit={assign}>
                <span className={styles.assignmentStep}>2</span>
                <div className={styles.assignmentCardHead}>
                  <h3>Ajouter un intervenant spécialisé</h3>
                  <p>Seulement pour une matière confiée à une autre personne.</p>
                </div>
                <label>Classe<select name="classId" required><option value="">Sélectionner une classe</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
                <label>Matière concernée<select name="subjectId" required><option value="">Sélectionner une matière</option>{activeSubjects.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
                <label>Enseignant spécialisé<select name="teacherId" required><option value="">Sélectionner un enseignant</option>{eligibleTeachers.map((item) => <option key={item.id} value={item.id}>{item.firstName} {item.lastName}</option>)}</select></label>
                <button className={styles.assignmentSecondaryButton}>Ajouter l’intervention</button>
              </form>
            </div>
          ) : (
            <form className={styles.assignmentSingleForm} onSubmit={assign}>
              <label>Classe<select name="classId" required><option value="">Sélectionner une classe</option>{classes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
              <label>Matière<select name="subjectId" required><option value="">Sélectionner une matière</option>{activeSubjects.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
              <label>Enseignant<select name="teacherId" required><option value="">Sélectionner un enseignant</option>{eligibleTeachers.map((item) => <option key={item.id} value={item.id}>{item.firstName} {item.lastName}</option>)}</select></label>
              <label className={styles.assignmentCheck}><input type="checkbox" name="headTeacher" /> Enseignant principal</label>
              <button className={styles.assignmentPrimaryButton}>Enregistrer l’affectation</button>
            </form>
          )}

          {primary ? (
            <section className={styles.classAssignments}>
              <header>
                <div><h2>Répartition des classes</h2><p>Une ligne claire par classe, sans répéter toutes les matières du titulaire.</p></div>
                <span>{classesWithTitular.size}/{classes.length} classe(s) pourvue(s)</span>
              </header>
              <div className={styles.classAssignmentGrid}>
                {classes.map((item) => {
                  const titular = activeAssignments.find(
                    (assignment) => assignment.classId === item.id && assignment.headTeacher,
                  );
                  const classExceptions = exceptions.filter(
                    (assignment) => assignment.classId === item.id,
                  );
                  return (
                    <article key={item.id} className={titular ? styles.classAssigned : styles.classUnassigned}>
                      <header>
                        <div><small>Classe</small><h3>{item.name}</h3></div>
                        <span>{titular ? "Titulaire défini" : "À affecter"}</span>
                      </header>
                      <div className={styles.titularLine}>
                        <small>Enseignant titulaire</small>
                        <b>{titular ? userName(workspace.users, titular.teacherId) : "Aucun titulaire"}</b>
                      </div>
                      {classExceptions.length > 0 && (
                        <div className={styles.exceptionList}>
                          <small>Interventions spécialisées</small>
                          {classExceptions.map((assignment) => {
                            const subject = workspace.subjects.find((subjectItem) => subjectItem.id === assignment.subjectId);
                            return (
                              <div key={assignment.id}>
                                <span><b>{subject?.label || "Matière"}</b><small>{userName(workspace.users, assignment.teacherId)}</small></span>
                                <button type="button" onClick={() => void removeAssignment(assignment)}>Retirer</button>
                              </div>
                            );
                          })}
                        </div>
                      )}
                      {titular && <button type="button" className={styles.removeTitular} onClick={() => void removePrimaryClassTeacher(item.id)}>Retirer le titulaire</button>}
                    </article>
                  );
                })}
              </div>
            ...
            <DataTable
              headers={["Matière", "Classe", "Enseignant", "Principal", "Action"]}
              rows={activeAssignments.map((assignment) => {
                ...
                return [
                  ...
                  <button key={assignment.id} className={`${styles.button} ${styles.buttonDanger}`} onClick={() => void removeAssignment(assignment)}>Retirer</button>,
                ];
              })}
            />
          )}
        ...
        <div className={styles.subjectWorkspace}>
          <section className={styles.subjectSetup}>
            <div>
              <small>Configuration</small>
              <h2>Matières de l’établissement</h2>
              <p>{activeSubjects.length} matière(s) active(s). Cette liste n’apparaît plus dans le formulaire des affectations.</p>
            </div>
            <button type="button" className={`${styles.button} ${styles.buttonSecondary}`} onClick={() => void installSuggestedSubjects()} disabled={!missingSuggestedSubjects.length}>
              {missingSuggestedSubjects.length ? `Installer ${missingSuggestedSubjects.length} matière(s) manquante(s)` : "Profil complet"}
            </button>
          </section>

          <form className={`${styles.card} ${styles.form} ${styles.subjectCreationForm}`} onSubmit={addSubject}>
            <h2>Ajouter une matière</h2>
            <div className={styles.two}>
              <label>Code<input name="code" required /></label>
              <label>Libellé<input name="label" required list="school-subject-suggestions" placeholder={suggestedSubjects[0] || "Français"} /></label>
            </div>
            <div className={styles.three}>
              <label>Coefficient<input name="coefficient" type="number" min="0.1" step="0.1" defaultValue="1" /></label>
              <label>Heures/semaine<input name="weeklyHours" type="number" min="0" step="0.5" /></label>
              <label>Couleur<input name="color" type="color" defaultValue="#08734f" /></label>
            </div>
            <input type="hidden" name="levelId" value={workspace.levels[0]?.id || ""} />
            <div className={styles.actions}><button className={styles.button}>Ajouter la matière</button></div>
          </form>

          <DataTable
            headers={["Matière", "Code", "Coefficient", "Volume", "Affectations", "Action"]}
            rows={activeSubjects.map((subject) => {
              const used = activeAssignments.filter((item) => item.subjectId === subject.id).length;
              return [
                subject.label,
                subject.code,
                subject.coefficient,
                ...
                <div key={subject.id} className={styles.actions}>
                  <button className={`${styles.button} ${styles.buttonSecondary}`} onClick={() => void editSubject(subject)}>Modifier</button>
                  <button className={`${styles.button} ${styles.buttonDanger}`} onClick={() => void removeSubject(subject)}>Supprimer</button>
                </div>,
              ];
            })}
          />
