...
export async function printReceiptInIsolatedWindow(data:FinanceReceiptData){
  ...
  const printButton=doc.getElementById("print-finance-receipt");
  if(!printButton){target.close();throw new Error("Le bouton d’impression du reçu est indisponible.")}
  printButton.addEventListener("click",()=>{target.focus();target.print()});
  target.focus();
}
