...
describe("calculs financiers FCFA", () => {
  ...
  it("écarte successivement établissement, niveau et classe lorsqu’un barème plus précis existe",()=>{const students=[{id:"a",active:true,schoolId:"e",gradeLevelId:"l",classGroupId:"c"}];const scales=[{id:"s",scopeType:"school" as const},{id:"l",scopeType:"level" as const,gradeLevelId:"l"},{id:"c",scopeType:"class" as const,classGroupId:"c"},{id:"a",scopeType:"student" as const,studentId:"a"}];for(const target of scales.slice(0,3)){const preview=previewCollectiveAssignment(students,{...target,schoolId:"e",amountFcfa:10},scales,[]);expect(preview.overshadowedIds).toEqual(["a"]);expect(preview.pendingIds).toEqual([]);}expect(previewCollectiveAssignment(students,{...scales[3],schoolId:"e",amountFcfa:10},scales,[]).pendingIds).toEqual(["a"]);});
  it("signale une ancienne charge moins prioritaire sans la remplacer",()=>{const students=[{id:"a",active:true,schoolId:"e",gradeLevelId:"l",classGroupId:"c"}];const precise={id:"a",scopeType:"student" as const,studentId:"a",schoolId:"e",amountFcfa:10};const preview=previewCollectiveAssignment(students,precise,[{id:"s",scopeType:"school"},precise],[{studentId:"a",sourceScaleId:"s"}]);expect(preview.conflictIds).toEqual(["a"]);expect(preview.pendingIds).toEqual([]);});
});
