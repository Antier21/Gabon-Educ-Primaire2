...
describe("PDF professionnel du reçu financier",()=>{
  ...
  it("préserve l’impression du récapitulatif de caisse",()=>{expect(css).toContain(".panel>*:not(.cashPrint)");expect(css).toContain(".cashPrint{display:block!important");expect(manager).toContain('onClick={()=>window.print()}');});
