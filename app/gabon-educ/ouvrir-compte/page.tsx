...
import { SchoolActivationGuard } from "@/components/SchoolActivationGuard";
...
export default async function OpenAccountPage({ searchParams }: Props) {
  ...
  return (
    <SchoolActivationGuard profileKey={profileKey}>
      <main className="account-opening-page">
        <section className="account-opening-card" aria-labelledby="account-opening-title">
          <Link className="back" href={`/gabon-educ/activation-etablissement?profile=${profileKey}`}>← Retour à l’activation</Link>
          ...
          <div className="account-opening-steps" aria-label="Étapes d’inscription">
            <span><CheckCircle2 /> 1. Autorisation GEPS validée</span>
            <span><UserPlus /> 2. Créer le compte responsable</span>
            <span><FileText /> 3. Enregistrer l’établissement</span>
          </div>
          <Link className="btn btn-primary btn-large full" href={`/gabon-educ/inscription?profile=${profileKey}`}>
            Continuer vers l’ouverture du compte <ArrowRight aria-hidden="true" />
          </Link>
        </section>
      </main>
    </SchoolActivationGuard>
  );
}
