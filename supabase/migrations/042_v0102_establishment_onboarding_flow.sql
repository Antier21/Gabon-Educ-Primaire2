-- Gabon Educ+ v0.10.2 — parcours d'inscription des établissements
-- Choix du profil -> ouverture de compte -> enregistrement établissement -> connexion administration
...
set school_type = case
  when lower(coalesce(school_type,'')) in ('primary','primaire','ecole primaire','école primaire') then 'primary'
  when lower(coalesce(school_type,'')) in ('high_school','lycee','lycée','lycee general','lycée général') then 'high_school'
  when lower(coalesce(school_type,'')) in ('complex_school','complexe','complexe scolaire') then 'complex_school'
  when lower(coalesce(school_type,'')) in ('middle_school','college','collège','college general','collège général','secondaire') then 'middle_school'
  else 'middle_school'
end;

update public.schools
set school_sector = case
  when lower(coalesce(school_sector,'')) = 'public' then 'public'
  else 'private'
end;
...
alter table public.schools add constraint schools_school_type_check
  check (school_type in ('primary', 'middle_school', 'high_school', 'complex_school'));
...
declare
  ...
  normalized_type text := coalesce(requested_school_type, 'middle_school');
  normalized_sector text := coalesce(requested_school_sector, 'private');
  generated_slug text;
  level_code text;
  year_start integer;
begin
  ...
  if normalized_sector not in ('public','private') then
    normalized_sector := 'private';
  end if;

  generated_slug := regexp_replace(lower(coalesce(school_name, 'etablissement')), '[^a-z0-9]+', '-', 'g');
  generated_slug := trim(both '-' from generated_slug);
  if generated_slug = '' then generated_slug := 'etablissement'; end if;
  ...
  insert into public.schools(
    ...
    nullif(registration_number, ''), nullif(province_name, ''), nullif(city_name, ''),
    nullif(school_address, ''), nullif(school_phone, ''), nullif(school_email, ''), true
  ...
  year_start := nullif(split_part(coalesce(academic_year_label, '2026-2027'), '-', 1), '')::integer;
  if year_start is null then year_start := extract(year from current_date)::integer; end if;

  insert into public.academic_years(school_id, label, starts_on, ends_on, is_current, is_archived)
  values (
    created_school_id,
    coalesce(nullif(academic_year_label, ''), year_start::text || '-' || (year_start + 1)::text),
