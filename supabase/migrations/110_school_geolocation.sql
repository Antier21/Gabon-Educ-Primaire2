-- Gabon ├ëduc+ Primaire ÔÇö g├®olocalisation des ├®tablissements dans le centre de pilotage
-- Migration volontairement isol├®e : elle peut ├¬tre appliqu├®e manuellement sans rejouer
-- l'historique divergent des migrations pr├®c├®dentes.
...
comment on column public.schools.latitude is 'Latitude GPS de l ├®tablissement, d├®finie par le super administrateur.';
comment on column public.schools.longitude is 'Longitude GPS de l ├®tablissement, d├®finie par le super administrateur.';
...
begin
  if auth.uid() is null or not public.is_super_admin() then
    raise exception 'Acc├¿s r├®serv├® au super administrateur.' using errcode = '42501';
  end if;

  if p_school_id is null then
    raise exception '├ëtablissement absent.' using errcode = '22023';
  end if;

  if (p_latitude is null) <> (p_longitude is null) then
    raise exception 'Latitude et longitude doivent ├¬tre renseign├®es ensemble.' using errcode = '22023';
  end if;
  ...
  if not found then
    raise exception '├ëtablissement introuvable.' using errcode = 'P0002';
  end if;
