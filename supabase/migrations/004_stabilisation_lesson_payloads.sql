-- Gabon Éduc+ v0.6.3 — persistance complète des fiches APC
alter table public.lesson_plans add column if not exists payload jsonb not null default '{}'::jsonb;
...
begin
  ...
  if v_profile_id is null then raise exception 'Profil utilisateur introuvable'; end if;

  select id into v_subject_id from public.subjects where lower(name) = lower(coalesce(p_payload->>'subject','Français')) limit 1;
  select id into v_grade_id from public.grade_levels where lower(name) = lower(coalesce(p_payload->>'grade','5e')) limit 1;
  if v_subject_id is null or v_grade_id is null then raise exception 'Matière ou niveau introuvable'; end if;

  insert into public.lesson_plans(
    id, teacher_id, subject_id, grade_level_id, title, week_number, duration_minutes,
    prerequisite, situation_problem, lesson_summary, differentiation, homework, status, payload
  ) values (
    ...
    p_payload->>'prerequisite', p_payload->>'situationProblem', p_payload->>'summary',
    p_payload->>'differentiation', p_payload->>'homework',
...
begin
  ...
  where lp.id = p_id and lp.teacher_id = auth.uid();
end;
$$;
